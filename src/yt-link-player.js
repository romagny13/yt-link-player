...
class YouTubeLinkPlayer {
  constructor(options = {}) {
    ...
    this._tooltips = new Set(); // Tracker pour les tooltips actifs
    this._options = {
      ...
      tooltipDelay: 300, // Délai avant affichage du tooltip (ms)
  ...
  _isVideoVisible(link) {
    ...
    if (videoData) {
      return videoData.container.classList.contains(this._options.videoVisibleClass);
    }
  ...
  _addPreviewTooltip(link) {
    const thumbnailUrl = this.getThumbnailUrl(link.href);
    if (!thumbnailUrl) {
      console.warn(
    ...
    // Préchargement et gestion du cache
    ...
    const showTooltip = (e) => {
      ...
      const tooltipWidth = parseInt(width);
      const tooltipHeight = parseInt(height);
